...
from sos2_storage_with_soc_dependent_efficiency import Storage as Stor
...
def read_scenario_parameters(scenario_options, i_scenario):
    ...
    soc_breakpoints = [
        float(x)
        for x in scenario_options.at[i_scenario, "soc_breakpoints_[-]"].split(", ")
    ]
    soc_loss_breakpoints = [
        float(x)
        for x in scenario_options.at[i_scenario, "soc_loss_breakpoints_[-]"].split(", ")
    ]
...
def energy_system_model(
    ...
    storage = Stor(
        ...
        soc_loss_breakpoints=soc_loss_breakpoints,
