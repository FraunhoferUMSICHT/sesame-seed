...
from linear_storage import Storage as Stor
...
def read_scenario_parameters(scenario_options, i_scenario):
    ...
    ETA_IN = scenario_options.at[i_scenario, "ETA_IN_[-]"]
    ETA_OUT = scenario_options.at[i_scenario, "ETA_OUT_[-]"]

    sf_dem = scenario_options.at[i_scenario, "sf_dem_[-]"]
    sf_res = scenario_options.at[i_scenario, "sf_res_[-]"]


    return (P_MAX_IN, P_MAX_OUT, SOC_MIN, SOC_MAX, SOC_INI, ETA_IN, ETA_OUT, sf_dem, sf_res)
...
def energy_system_model(timesteps, demand, ee_generation, P_MAX_IN, P_MAX_OUT, SOC_MIN, SOC_MAX, SOC_INI, ETA_IN, ETA_OUT):
...
def set_up_energy_system_model(scenario_options, i_scenario):
    ...
    (P_MAX_IN, P_MAX_OUT, SOC_MIN, SOC_MAX, SOC_INI, 
    ETA_IN, ETA_OUT, sf_dem, sf_res) = read_scenario_parameters(scenario_options, i_scenario)

    # read timeseries data
    demand, ee_generation = read_timeseries(scenario_options, i_scenario, sf_dem, sf_res)
    
    # create optimization model
    model = energy_system_model(timesteps, demand, ee_generation, P_MAX_IN, P_MAX_OUT, 
                                SOC_MIN, SOC_MAX, SOC_INI, ETA_IN, ETA_OUT)
    
    return model


def process_solver_results(solver_results, model, scenario_status, i_scenario, export_root, scenario_options):
    ...
    if (solver_results.solver.status == SolverStatus.ok) or (
        ...
        scenario_status.at[i_scenario, "eta_in_mean"] = scenario_options.at[i_scenario, "ETA_IN_[-]"]
        scenario_status.at[i_scenario, "eta_out_mean"] = scenario_options.at[i_scenario, "ETA_OUT_[-]"]
